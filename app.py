...
if st.session_state["current_page"] == "투자 내역 입력"]:
    ...
    with col1:
        if st.button("➕ 추가"):
            st.session_state["repayment_data"].append({"회차": len(st.session_state["repayment_data"]) + 1, "지급예정일": None, "원금": 0, "이자": 0, "세금": 0, "수수료": 0, "상환완료": False})
            st.rerun()
    with col2:
    ...
    if st.session_state["edit_mode"] and st.button("저장"):
        st.session_state["edit_mode"] = False
